...
export async function update() {
    ...
    const res = await Unwrap(PlexcordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(PlexcordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(PlexcordNative.updater.getRepo());
