...
function PlexcordPopout(onClose: () => void) {
    const { useQuickCss } = useSettings(["useQuickCss"]);

    const pluginEntries = [] as ReactNode[];

    for (const plugin of Object.values(Plexcord.Plugins.plugins)) {
        if (plugin.toolboxActions && Plexcord.Plugins.isPluginEnabled(plugin.name)) {
            pluginEntries.push(
                <Menu.MenuGroup
                    label={plugin.name}
                    key={`pc-toolbox-${plugin.name}`}
                >
                    {Object.entries(plugin.toolboxActions).map(([text, action]) => {
                        const key = `pc-toolbox-${plugin.name}-${text}`;
...
function PlexcordPopoutIcon(isShown: boolean) {
    return (
...
function PlexcordPopoutButton() {
    ...
    return (
        <Popout
            ...
            renderPopout={() => PlexcordPopout(() => setShow(false))}
        >
            {(_, { isShown }) => (
                <HeaderBarIcon
                    ref={buttonRef}
