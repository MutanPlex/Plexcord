...
import { Message, MessageAttachment, ScrollerBaseRef } from "@plexcord/discord-types";
    ...
    return (
        <div id="files-picker-tab-panel" role="tabpanel" aria-labelledby="files-picker-tab" className={cl("container")}>
            <div className={cl("container-header")}>
                <ManaSearchBar
                    autoFocus
                    placeholder={t(plugin.favouriteAnything.searchPlaceholder)}
                    query={query}
...
function EmptyList() {
    return <BaseText className={cl("info-text")}>{t(plugin.favouriteAnything.noFiles)}</BaseText>;
}
    ...
    return (
        <>
            ...
            <BaseText className={cl("info-text")}>
                {t(plugin.favouriteAnything.demoText)}
            </BaseText>
        </>
    );
}
