...
import { Channel, MessageAttachment } from "@plexcord/discord-types";
...
const Native = PlexcordNative.pluginHelpers.FavouriteAnything as PluginNative<typeof import("./native")>;

export const cl = classNameFactory("pc-favouriteAnything-");
...
const fallbackThumbnail = new URL("https://raw.githubusercontent.com/MutanPlex/random-files/refs/heads/main/invalid.jpg");
...
export async function sendAttachment(attachment: MessageAttachment, channel: Channel) {
    ...
    const file = await fetchAttachment(attachment).catch(() =>
        Toasts.show({ message: t(plugin.favouriteAnything.fetchError, { filename: attachment.filename }), id: Toasts.genId(), type: Toasts.Type.FAILURE })
    );
    if (!file) return;
    ...
    await UploadHandler.promptToUpload([file], channel, DraftType.ChannelMessage).catch(() =>
        Toasts.show({ message: t(plugin.favouriteAnything.uploadError, { filename: attachment.filename }), id: Toasts.genId(), type: Toasts.Type.FAILURE })
    );
