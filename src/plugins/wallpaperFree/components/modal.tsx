...
export function SetDiscordWallpaperModal({ props, onSelect }: Props) {
    ...
    return (
        <ModalRoot {...props} size={ModalSize.MEDIUM}>
            ...
            <ModalContent>
                <div className="pc-wpfree-discord-wp-modal">
                    {lodash.chunk(discordWallpapers, 2).map(group => {
                        const main = group[0];
                        return (
                            <div key={main.id} className="pc-wpfree-discord-wp-icon-container">
                                <figure style={{ margin: 0, textAlign: "center" }}>
                                    <img
                                        className="pc-wpfree-discord-wp-icon-img"
                                ...
                                <div className="pc-wpfree-discord-set-buttons">
                                    {group.map(wp => (
                                        <Button
                                            key={wp.id}
