...
const settings = definePluginSettings({
    userOutput: {
        label: () => t(plugin.dragify.option.userOutput.label),
        description: () => t(plugin.dragify.option.userOutput.description),
        type: OptionType.SELECT,
        options: [
            { label: () => t(plugin.dragify.option.userOutput.mention), value: "mention", default: true },
            { label: () => t(plugin.dragify.option.userOutput.id), value: "id" },
        ],
    },
    channelOutput: {
        label: () => t(plugin.dragify.option.channelOutput.label),
        description: () => t(plugin.dragify.option.channelOutput.description),
        type: OptionType.SELECT,
        options: [
            { label: () => t(plugin.dragify.option.channelOutput.mention), value: "mention", default: true },
            { label: () => t(plugin.dragify.option.channelOutput.link), value: "link" },
        ],
    },
    inviteExpireAfter: {
        label: () => t(plugin.dragify.option.inviteExpireAfter.label),
        description: () => t(plugin.dragify.option.inviteExpireAfter.description),
        type: OptionType.SELECT,
        options: [
            { label: () => t(plugin.dragify.option.inviteExpireAfter.thirtyMinutes), value: 1800 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.oneHour), value: 3600 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.sixHours), value: 21600 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.twelveHours), value: 43200 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.oneDay), value: 86400 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.sevenDays), value: 604800 },
            { label: () => t(plugin.dragify.option.inviteExpireAfter.never), value: 0, default: true },
        ],
    },
    inviteMaxUses: {
        label: () => t(plugin.dragify.option.inviteMaxUses.label),
        description: () => t(plugin.dragify.option.inviteMaxUses.description),
        type: OptionType.SELECT,
        options: [
            { label: () => t(plugin.dragify.option.inviteMaxUses.noLimit), value: 0, default: true },
            { label: () => t(plugin.dragify.option.inviteMaxUses.one), value: 1 },
            { label: () => t(plugin.dragify.option.inviteMaxUses.five), value: 5 },
            { label: () => t(plugin.dragify.option.inviteMaxUses.ten), value: 10 },
            { label: () => t(plugin.dragify.option.inviteMaxUses.twentyFive), value: 25 },
            { label: () => t(plugin.dragify.option.inviteMaxUses.fifty), value: 50 },
            { label: () => t(plugin.dragify.option.inviteMaxUses.hundred), value: 100 },
        ],
    },
    inviteTemporaryMembership: {
        label: () => t(plugin.dragify.option.inviteTemporaryMembership.label),
        description: () => t(plugin.dragify.option.inviteTemporaryMembership.description),
        type: OptionType.BOOLEAN,
        default: false,
    },
    reuseExistingInvites: {
        label: () => t(plugin.dragify.option.reuseExistingInvites.label),
        description: () => t(plugin.dragify.option.reuseExistingInvites.description),
        type: OptionType.BOOLEAN,
        default: false,
    },
    allowChatBodyDrop: {
        label: () => t(plugin.dragify.option.allowChatBodyDrop.label),
        description: () => t(plugin.dragify.option.allowChatBodyDrop.description),
        type: OptionType.BOOLEAN,
        default: false,
        restartNeeded: true,
    },
...
export default definePlugin({
    ...
    async handleDropEntity(entity: DropEntity, channel: Channel) {
        try {
            ...
            showToast(t(plugin.dragify.toast.failed.drop), Toasts.Type.FAILURE);
        }
    },
    ...
    async createInvite(guildId: string, currentChannel: Channel): Promise<string | null> {
        ...
        if (!inviteChannelId) {
            showToast(t(plugin.dragify.toast.invite.noChannel), Toasts.Type.FAILURE);
            return null;
        }
        ...
        try {
            ...
            showToast(t(plugin.dragify.toast.invite.created), Toasts.Type.SUCCESS);
            return `https://discord.gg/${code}`;
        } catch (error) {
            logger.error("Failed to create invite", error);
            showToast(t(plugin.dragify.toast.invite.unable), Toasts.Type.FAILURE); // uh oh!
            return null;
        }
    },
    ...
    buildGhost(entity: DropEntity): Omit<GhostState, "visible" | "x" | "y"> | null {
        if (entity.kind === "user") {
            ...
            const subtitle = user.username ? `@${user.username}` : t(plugin.dragify.ghost.user);
            const iconUrl = user.getAvatarURL(void 0, 80, true) ?? undefined;
            return { kind: "user", title, subtitle, iconUrl, symbol: "@", badge: t(plugin.dragify.ghost.badge.user), entityId: entity.id, exiting: false };
        }
        ...
        if (entity.kind === "channel") {
            ...
            const subtitle = guild?.name ?? (entity.guildId ? t(plugin.dragify.ghost.server) : t(plugin.dragify.ghost.dm));
            ...
            let badge = t(plugin.dragify.ghost.badge.channel);
            if (isThread) {
                badge = t(plugin.dragify.ghost.badge.thread);
            } else {
                switch (channel.type) {
                    case ChannelType.GUILD_VOICE:
                    case ChannelType.GUILD_STAGE_VOICE:
                        badge = t(plugin.dragify.ghost.badge.voice);
                        break;
                    case ChannelType.GUILD_FORUM:
                        badge = t(plugin.dragify.ghost.badge.forum);
                        break;
                    case ChannelType.GUILD_MEDIA:
                        badge = t(plugin.dragify.ghost.badge.media);
                        break;
                    case ChannelType.GUILD_ANNOUNCEMENT:
                        badge = t(plugin.dragify.ghost.badge.announcement);
                        break;
                    case ChannelType.DM:
                    case ChannelType.GROUP_DM:
                        badge = t(plugin.dragify.ghost.badge.dm);
                        break;
                    default:
                        badge = t(plugin.dragify.ghost.badge.channel);
                }
            }
        ...
        const title = guild?.name ?? t(plugin.dragify.ghost.server);
        const subtitle = t(plugin.dragify.ghost.server);
        const iconUrl = guild?.icon
            ? IconUtils.getGuildIconURL({ id: guild.id, icon: guild.icon, size: 64 }) ?? undefined
            : undefined;
        const symbol = guild ? getGuildAcronym(guild) : "S";
        return { kind: "guild", title, subtitle, iconUrl, symbol, badge: t(plugin.dragify.ghost.badge.server), entityId: entity.id, exiting: false };
    },
