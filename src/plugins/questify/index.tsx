...
const QuestifyNative = PlexcordNative.pluginHelpers.Questify as PluginNative<typeof import("./native")>;
            ...
            {activeQuestIntervals.has(props.quest.id) ? (
                ...
                />) : canAutoComplete ? (
                    <Menu.MenuItem
                        ...
                        label={t(plugin.questify.context.quest.startAuto)}
                        action={() => { processQuestForAutoComplete(props.quest, true); }}
                    />) : null
            }
            <Menu.MenuItem
                id={q("copy-quest-id")}
                label={t(plugin.questify.context.quest.copyQuestID)}
...
async function startVideoProgressTracking(quest: Quest, target: { raw: number; adjusted: number; }): Promise<void> {
    ...
    async function handleSendComplete(): Promise<void> {
        clearTrackingTimers();
    ...
    function scheduleNextPlaybackTick(): void {
    ...
    {
        const intervalData = activeQuestIntervals.get(quest.id);
        if (intervalData && renderIntervalId) {
...
function processQuestForAutoComplete(quest: Quest, force: boolean = false): boolean {
    ...
    return false; // true means continue as normal, false means prevent default action.
}
...
function getQuestUnacceptedButtonText(quest: Quest): string | null {
    ...
    if ((isPlay && completeGameQuestsInBackground && IS_DISCORD_DESKTOP) || (isWatch && completeVideoQuestsInBackground && target > 0)) {
        return t(plugin.questify.button.complete, { remainTime: targetFormatted });
    } else if ((isAchievement && completeAchievementQuestsInBackground) || (isWatch && completeVideoQuestsInBackground && target === 0)) {
        return t(plugin.questify.button.completeImmediate);
    }
        ...
        if (((isPlay && completeGameQuestsInBackground && IS_DISCORD_DESKTOP) || (isWatch && completeVideoQuestsInBackground))) {
            ...
            if (!!intervalData) {
                return canCompleteImmediately || !timeRemaining
                    ? t(plugin.questify.button.completing) + "..."
                    : prepositional ? t(plugin.questify.button.questInProgressWithTime, { remainTime: progressFormattedAsPreposition }) : t(plugin.questify.button.questInProgressWithTime, { remainTime: progressFormatted });
            } else if (isWatch || isPlay) {
                return canCompleteImmediately ? t(plugin.questify.button.completeImmediate) : timeRemaining === durationWithLeeway ? t(plugin.questify.button.complete, { remainTime: progressFormatted }) : t(plugin.questify.button.resume, { remainTime: progressFormatted });
            }
        } else if (isAchievement && completeAchievementQuestsInBackground) {
            if (!!intervalData) {
                return t(plugin.questify.button.completing);
            } else {
                return t(plugin.questify.button.completeImmediate);
            }
        }
...
function getQuestPanelSubtitleText(quest: Quest): string | null {
    ...
    const completingText = getQuestAcceptedButtonText(quest, true) || t(plugin.questify.button.completing);
    const completedText = questCompleted ? t(plugin.questify.button.completed) : null;
    ...
    if (rewardItem.orbQuantity) {
        return t(plugin.questify.reward.orbs, { completingText, orbQuantity: rewardItem.orbQuantity });
    } else if (rewardItem?.messages?.nameWithArticle) {
        return t(plugin.questify.reward.article, { completingText, itemName: rewardItem.messages.nameWithArticle });
    } else {
        return t(plugin.questify.reward.unrecognized, { completingText });
    }
}
