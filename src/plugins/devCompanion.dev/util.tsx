...
import { plugin as pluginLang, t } from "@api/i18n";
...
export function toggleEnabled(name: string, beforeReload: (error?: string) => void) {
    ...
    const settings = Settings.plugins[typeof plugin.name === "function" ? plugin.name() : plugin.name];
    ...
    if (!wasEnabled) {
        ...
        if (failures.length) {
            ...
            showNotice(t(pluginLang.devCompanion.toast.failed, { failures: failures.join(", ") }), t(pluginLang.devCompanion.toast.close), () => null);
            beforeReturn();
            return;
    ...
    if (!result) {
        ...
        const msg = wasEnabled ? t(pluginLang.devCompanion.toast.stopping, { plugin: plugin.name }) : t(pluginLang.devCompanion.toast.starting, { plugin: plugin.name });
        console.error(msg);
        showErrorToast(msg);
        beforeReturn();
        return;
