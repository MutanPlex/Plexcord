...
const settings = definePluginSettings({
    typeOfSnow: {
        label: () => t(plugin.snowfall.option.typeOfSnow.label),
        description: () => t(plugin.snowfall.option.typeOfSnow.description),
        type: OptionType.SELECT,
        options: [
            { label: () => t(plugin.snowfall.option.typeOfSnow.solid), value: "solid" },
            { label: () => t(plugin.snowfall.option.typeOfSnow.text), value: "text", default: true },
            { label: () => t(plugin.snowfall.option.typeOfSnow.emoji), value: "image" }
        ],
    },
    maxSize: {
        label: () => t(plugin.snowfall.option.maxSize.label),
        description: () => t(plugin.snowfall.option.maxSize.description),
        type: OptionType.SLIDER,
        default: 30,
        markers: [10, 20, 30, 40, 50]
    },
    speed: {
        label: () => t(plugin.snowfall.option.speed.label),
        description: () => t(plugin.snowfall.option.speed.description),
        type: OptionType.SLIDER,
        default: 50,
        markers: [50, 100, 200, 300, 400, 500]
    },
    flakesPerSecond: {
        label: () => t(plugin.snowfall.option.flakesPerSecond.label),
        description: () => t(plugin.snowfall.option.flakesPerSecond.description),
        type: OptionType.SLIDER,
        default: 5,
...
export default definePlugin({
    ...
    settingsAboutComponent: () => (
        <>
            <Heading>{t(plugin.snowfall.about.title)}</Heading>
            <Paragraph>
                {t(plugin.snowfall.about.paragraph)}
                <br /><br />
                {t(plugin.snowfall.about.note)}
            </Paragraph>
        </>
    ),
