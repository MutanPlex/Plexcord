    ...
    const showImportPrompt = (existingCount: number): Promise<ImportDecision> => {
        return new Promise(resolve => {
            openModal(props => (
                <ImportProfilesModal
                    {...props}
                    title={t(plugin.profileSets.modal.importProfiles)}
                    message={t(plugin.profileSets.modal.importText, { existingCount })}
    ...
    return (
        <div className={classes(cl("section"), isServerSection ? cl("section-server") : "")} >
            <Heading tag="h3" className={cl("heading")}>
                {t(plugin.profileSets.modal.heading)}
            </Heading>

            <div className={cl("text")}>
                <TextInput
                    placeholder={searchMode ? t(plugin.profileSets.modal.searchProfiles) : t(plugin.profileSets.modal.profileName)}
                    value={presetName}
            ...
            <div className={cl("search")}>
                {!searchMode && (
                    ...
                    >
                        {isSaving ? t(plugin.profileSets.modal.saving) : t(plugin.profileSets.modal.save)}
                    </Button>
                )}
                {hasPresets && (
                    <Button
                    ...
                    >
                        {searchMode ? t(plugin.profileSets.modal.cancelSearch) : t(plugin.profileSets.modal.search)}
                    </Button>
                )}
            </div>
            ...
            <div className={cl("import")}>
                ...
                >
                    <span className={cl("random-content")}>
                        ...
                        {t(plugin.profileSets.modal.random)}
                    </span>
                </Button>
                <Button
                    size="small"
                ...
                >
                    {t(plugin.profileSets.modal.importProfiles)}
                </Button>
                <Button
                    size="small"
                    variant="secondary"
                ...
                >
                    {t(plugin.profileSets.modal.exportAll)}
                </Button>
            </div>
            <hr className={cl("block")} />
