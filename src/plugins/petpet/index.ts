...
function loadImage(source: File | string) {
    ...
    return new Promise<HTMLImageElement>((resolve, reject) => {
        ...
        img.onerror = _event => reject(Error(t(plugin.petpet.error.fetchImageFailed, { url })));
    ...
    commands: [
        {
            ...
            execute: async (opts, cmdCtx) => {
                ...
                if (delay < 20) return sendBotMessage(cmdCtx.channel.id, { author: { username: "Plexcord" }, content: t(plugin.petpet.command.petpet.error.delayTooLow) });
