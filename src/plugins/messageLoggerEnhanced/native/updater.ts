...
const PLEXCORD_USER_PLUGIN_DIR = join(__dirname, "..", "src", "userplugins");
const getCwd = memoize(async () => {
    const dirs = await readdir(PLEXCORD_USER_PLUGIN_DIR, { withFileTypes: true });

    for (const dir of dirs) {
        if (!dir.isDirectory()) continue;

        const pluginDir = join(PLEXCORD_USER_PLUGIN_DIR, dir.name);
        const files = await readdir(pluginDir);

        if (files.includes("LoggedMessageManager.ts")) return join(PLEXCORD_USER_PLUGIN_DIR, dir.name);
    }
