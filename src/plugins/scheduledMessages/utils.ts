...
export function getChannelDisplayInfo(channelId: string): { name: string; avatar: string; } {
    ...
    if (!channel) return { name: t(plugin.scheduledMessages.channelType.unknown), avatar: "" };
    ...
    if (channel.isDM()) {
        ...
        return user
            ...
            : { name: t(plugin.scheduledMessages.channelType.dm), avatar: "" };
    }

    if (channel.isGroupDM() || channel.isMultiUserDM()) {
        return { name: channel.name || t(plugin.scheduledMessages.channelType.groupDm), avatar: IconUtils.getChannelIconURL(channel) ?? "" };
    }

    const guild = GuildStore.getGuild(channel.guild_id);
    return {
        name: channel.name || t(plugin.scheduledMessages.channelType.channel),
...
function applyPhantomClassToMessage(channelId: string, messageId: string): void {
    const tryApply = (retries = 0) => {
        ...
        if (el) {
            el.classList.add("pc-scheduled-msg-phantom");
            return true;
        }
        if (retries < 20) {
...
async function sendScheduledMessage(msg: ScheduledMessage): Promise<boolean> {
    try {
        ...
        if (settings.store.showNotifications) {
            showToast(t(plugin.scheduledMessages.toast.messageSent, { channel: getChannelDisplayInfo(msg.channelId).name }), Toasts.Type.SUCCESS);
        }
        return true;
    } catch {
        if (settings.store.showNotifications) showToast(t(plugin.scheduledMessages.toast.messageFailed), Toasts.Type.FAILURE);
        return false;
    }
}
