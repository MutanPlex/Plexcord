...
import { Emoji, Message } from "@plexcord/discord-types";
...
const cl = classNameFactory("pc-emoji-alias-");
...
const settings = definePluginSettings({
    aliases: {
        ...
        label: () => t(plugin.favoriteEmojiFirst.option.aliases.label),
        description: () => t(plugin.favoriteEmojiFirst.option.aliases.description),
        component: AliasListSetting
    },
    clearAll: {
        type: OptionType.COMPONENT,
        label: () => t(plugin.favoriteEmojiFirst.option.clearAll.label),
        description: () => t(plugin.favoriteEmojiFirst.option.clearAll.description),
...
async function saveAlias(aliasInput: string, ref: StoredEmojiRef): Promise<{ ok: true; } | { ok: false; error: string; }> {
    ...
    if (existing && !isSameEmoji(existing, ref)) {
        return { ok: false, error: t(plugin.favoriteEmojiFirst.toast.duplicate) };
    }
    ...
    try {
        ...
        return { ok: false, error: t(plugin.favoriteEmojiFirst.toast.failedSave) };
    }
}

function getAliasMenuLabel(ref: StoredEmojiRef): string {
    return getExistingAliasForEmoji(ref) ? t(plugin.favoriteEmojiFirst.button.edit) : t(plugin.favoriteEmojiFirst.button.set);
}
...
async function removeAlias(alias: string) {
    ...
    try {
        ...
        Toasts.show({
            id: Toasts.genId(),
            message: t(plugin.favoriteEmojiFirst.toast.removed, { alias }),
        ...
        Toasts.show({
            id: Toasts.genId(),
            message: t(plugin.favoriteEmojiFirst.toast.failedRemove),
...
async function clearAliases() {
    ...
    try {
        ...
        Toasts.show({
            id: Toasts.genId(),
            message: t(plugin.favoriteEmojiFirst.toast.clearAll),
            type: Toasts.Type.SUCCESS
        });
    } catch (error) {
        ...
        Toasts.show({
            id: Toasts.genId(),
            message: t(plugin.favoriteEmojiFirst.toast.failedDeleted),
            type: Toasts.Type.FAILURE
        });
    }
}
    ...
    openModal(props => (
        <SetAliasModal
            ...
            onSave={async input => {
                ...
                Toasts.show({
                    id: Toasts.genId(),
                    message: t(plugin.favoriteEmojiFirst.toast.set, { emoji: getEmojiDisplayName(ref) }),
                    type: Toasts.Type.SUCCESS
                });
                return result;
            }}
...
function insertSetAliasMenuItem(children: Array<React.ReactElement<any> | null>, emojiRef: StoredEmojiRef) {
    const alreadyAdded = children.some(child => child?.props?.id === "pc-emoji-alias-set");
    if (alreadyAdded) return;

    const menuItem = (
        <Menu.MenuItem
            id="pc-emoji-alias-set"
