...
import type { Role } from "@plexcord/discord-types";
...
const cl = classNameFactory("pc-clickableroles-");
    ...
    return (
        <div className={cl("popout")} ref={setPopoutRef}>
            <div className={cl("header")}>
                ...
                <span className={cl("name")}>{role?.name ?? t(plugin.clickableRoles.modal.unknown)}</span>
                <span className={cl("count")}>{totalCount}</span>
            </div>
            <ScrollerThin className={cl("list")} fade>
                {loading ? (
                    <div className={cl("empty")}>{t(plugin.clickableRoles.modal.loading)}</div>
                ) : users.length === 0 ? (
                    <div className={cl("empty")}>{t(plugin.clickableRoles.modal.noMembers)}</div>
                ) : users.map(user => (
                    <div
                        key={user.id}
                        className={cl("user")}
