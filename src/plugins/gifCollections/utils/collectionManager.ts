...
// This cache exists because async + react class components don't play nice and stutters happen.
...
export async function refreshCacheCollection(): Promise<void> {
...
export async function moveGifToCollection(gifId: string, fromName: string, toName: string): Promise<void> {
    ...
    const updateCollectionMeta = (col: Collection) => {
        const latest = col.gifs.length
            ? col.gifs[col.gifs.length - 1].src
            : settings.store.defaultEmptyCollectionImage;
