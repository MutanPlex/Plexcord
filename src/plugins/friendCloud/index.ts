    ...
    commands: [
        {
            ...
            name: "friendcloud",
            description: () => t(plugin.friendCloud.command.friendcloud.description),
            options: [
                {
                    name: "count",
                    description: () => t(plugin.friendCloud.command.friendcloud.count),
            ...
            execute: async (opts, cmdCtx) => {
                ...
                if (!count) return sendBotMessage(cmdCtx.channel.id, { author: { username: "Plexcord" }, content: t(plugin.friendCloud.command.friendcloud.mustHigher) });
                ...
                try {
                    ...
                    if (!affinities?.length) {
                        return sendBotMessage(cmdCtx.channel.id, {
                            ...
                            content: t(plugin.friendCloud.command.friendcloud.noAffinities)
                        });
                    }
                    ...
                    if (!users.length) {
                        return sendBotMessage(cmdCtx.channel.id, {
                            ...
                            content: t(plugin.friendCloud.command.friendcloud.noValid)
                        });
                    }
                    ...
                    const drawImage = async (user: UserPosition) => {
                        try {
                            ...
                            if (loadedImages === totalImages) {
                                canvas.toBlob(blob => {
                                    if (!blob) {
                                        sendBotMessage(cmdCtx.channel.id, { author: { username: "Plexcord" }, content: t(plugin.friendCloud.command.friendcloud.genFailed) });
                                        return;
                                    }
                    ...
                    if (e instanceof Error)
                        sendBotMessage(cmdCtx.channel.id, { author: { username: "Plexcord" }, content: e.message });
                    else logger.error(e);
                }
            },
        },
