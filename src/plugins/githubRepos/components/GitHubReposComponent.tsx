...
export function GitHubReposComponent({ id, theme }: { id: string, theme: string; }) {
    ...
    if (loading) return <div className="pc-github-repos-loading">Loading repositories...</div>;
    if (error) return <div className="pc-github-repos-error">Error: {error}</div>;
    if (!repos.length) return null;

    if (returnJustButton) {
        return (
            <button
                className="pc-github-button"
    ...
    return (
        <div className="pc-github-repos-container">
            <div className="pc-github-repos-header">
                GitHub Repositories
                {userInfo && (
                    <span className="pc-github-repos-count">
                        {` (${topRepos.length}/${userInfo.totalRepos})`}
                    </span>
                )}
            </div>
            <Flex className="pc-github-repos-list" flexDirection="column">
                {topRepos.map(repo => (
                    <RepoCard
                        key={repo.id}
                        repo={repo}
            ...
            <div className="pc-github-repos-footer">
                <button
                    className="pc-github-repos-show-more"
                    onClick={openReposModal}
                >
                    Show More
                </button>
