...
function PlexcordChatBarButtons(props: ChatBarProps) {
...
addContextMenuPatch("textarea-context", (children, args) => {
    ...
    group.splice(idx, 0,
        <Menu.MenuItem id="pc-chat-buttons" key="plexcord-chat-buttons" label={"Plexcord" + t(plugins.metadata.api.chatButtons.context.buttons)}>
            {buttons.map(([id]) => (
                <Menu.MenuCheckboxItem
                    label={id}
                    key={id}
                    id={`pc-chat-button-${id}`}
